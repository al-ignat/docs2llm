...
Only shown when **no config file exists** (neither local `.docs2llm.yaml` nor global `~/.config/docs2llm/config.yaml`).

```
ℹ  Tip: run docs2llm init to save your preferences.
│
└  Done!
```
...
## 2. Init Wizard (`docs2llm init`)

Creates or updates config. Entry: `init.ts → runInit()`.
Use `--global` for `~/.config/docs2llm/config.yaml`, otherwise `.docs2llm.yaml`.
...
┌  docs2llm init
│
◆  Config found at .docs2llm.yaml. What would you like to do?
│  ● Add a template
│  ○ Edit defaults
...
```
ℹ  Config to write to .docs2llm.yaml:
│  defaults:
│    format: docx
│
│
...
## 3. Config Wizard (`docs2llm config`)
...
│  ● Local   .docs2llm.yaml
│  ○ Global  ~/.config/docs2llm/config.yaml
```
...
┌  docs2llm config
│
ℹ  Global: ~/.config/docs2llm/config.yaml
│  Local:  ./.docs2llm.yaml
...
```
ℹ  Config file: .docs2llm.yaml
│
└  Open it with your editor: $EDITOR ~/.docs2llm.yaml
```
...
```
$ docs2llm report.docx
✓ report.docx → report.md

$ docs2llm report.docx -f json -o ./out
✓ report.docx → ./out/report.json

$ docs2llm notes.md -t report
✓ notes.md → notes.docx
